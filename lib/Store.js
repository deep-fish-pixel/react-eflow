...
var storeNames = {
  ...
  getName: function getName(store) {
    if (store instanceof Store) {
      var _name = (0, _method.getMethodName)(store.constructor) || 'store',
          names = this.names[_name];
      if (!names) {
        names = this.names[_name] = [];
      }
      //首字母小写
      _name = _name.replace(/\b\w+\b/g, function (word) {
        return word.substring(0, 1).toLowerCase() + word.substring(1);
      });
      var storeName = _name + '_' + (names.length + 1);
  ...
  _createClass(Store, [{
    ...
    value: function initState(data) {
      ...
      for (var _name2 in data) {
        if (data.hasOwnProperty(_name2)) {
          state[_name2] = data[_name2];
        }
      }
    }
    ...
    * */
    ...
    value: function dispatch(method, value) {
      ...
      if (method) {
        ...
        process.env.NODE_ENV !== 'production' && (0, _invariant2['default'])(stateKey, '调用%s.dispatch 方法, 参数值%s 的stateKey为空, stateKey属性在构造函数中进行初始化, 应该是非原型方法', (0, _method.getMethodName)(this), name || 'method');
    ...
    * */
    ...
    value: function data(method, value) {
      var stateKey = method.stateKey;
      process.env.NODE_ENV !== 'production' && (0, _invariant2['default'])(stateKey, '调用%s.data 方法, 参数值%s 的stateKey为空, stateKey属性在构造函数中进行初始化, 应该是非原型方法', (0, _method.getMethodName)(this), name || 'method');
